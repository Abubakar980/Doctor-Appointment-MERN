...
const { doctors, currencySymbol, token, getDoctorsData, userData } = useContext(AppContext);
  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`http://localhost:3000/api/user/book-appointment`,{userId: userData._id, docId,slotDate,slotTime,},{headers: { token },});
      if(data.success){
      ...
      <div className="mt-12">
        ...
        <div className="mt-8">
          ...
          onClick={bookAppointment}
            disabled={!slotTime}
            className={`w-full sm:w-auto px-6 py-3 rounded-full text-white font-medium cursor-pointer
              ${slotTime
