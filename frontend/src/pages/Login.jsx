...
const Login = () => {
  ...
  const [state, setState] = useState('Sign Up')
    ...
    const onSubmitHandler = async (event)=>{
      ...
      try {
        if(state === 'Sign Up'){
          const {data} = await axios.post(`http://localhost:3000/api/user/register`, {name, password, email})
          ...
          const {data} = await axios.post(`http://localhost:3000/api/user/login`, {password, email})
  ...
  return (
    <form onSubmit={onSubmitHandler} className="min-h-[80vh] flex items-center">
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg">
        ...
        <button type="submit" className="bg-[#5f6fff] text-white w-full py-2 rounded-md cursor-pointer text-base">{state === 'Sign Up' ? "Create Account" : "Login"}</button>
        {
          state === "Sign Up"
